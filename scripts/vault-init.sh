...
KV_PATH="${VAULT_KV_PATH:-secret}"
KV_MOUNT="${VAULT_KV_MOUNT:-secret}"

# Vérifications de sécurité
if [ -z "$VAULT_TOKEN" ]; then
    echo "ERREUR: VAULT_TOKEN non défini."
    echo "Définissez VAULT_TOKEN avec un token Vault valide."
    echo "Ex: export VAULT_TOKEN='s.xxxxxxxxxxxxxx'"
    exit 1
fi

# Vérifier que le token n'est pas exposé dans l'environnement
if [ -t 0 ]; then
...
if ! vault status >/dev/null 2>&1; then
    ...
    echo "Vérifiez que Vault est démarré et que le token est valide."
    exit 1
fi
...
echo "Stockage des secrets dans Vault (chemin: $KV_MOUNT/$KV_PATH/music-hall)..."

# Stockage des secrets - les valeurs ne sont pas affichées
vault kv put -mount="$KV_MOUNT" "$KV_PATH/music-hall" \
    db_user="$DB_USER" \
...
echo "Chemin: $KV_MOUNT/$KV_PATH/music-hall"

# Ne JAMAIS afficher les valeurs des secrets
echo "Vérification de la structure (sans afficher les valeurs)..."
vault kv get -mount="$KV_MOUNT" "$KV_PATH/music-hall" --format=json 2>/dev/null | \
...
echo "- Le token Vault doit avoir les permissions appropriées"
